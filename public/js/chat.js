 ...
 */
 ...
 $('.bx-left-arrow-alt').click(function(){
  $('#dropdown-search-user').addClass('d-none');
 })
 var messenger,
 typingTimeout,
 typingNow = 0,
...
const messagesContainer = $('.messenger-layout .layout-main'),
...
function attachmentTemplate(fileType, fileName, imgURL = null) {
 if (fileType != "image") {
   ...
   const imageSrc = imgURL; // Use imgURL if provided, or an empty string
   return (
     `
   <div class="attachment-preview">
   <span class="fas fa-times cancel"></span>
   <div class="image-file chat-image" ><img style="
   width: 100%;
  " src="${imageSrc}" /></div>
   <p><span class="fas fa-file-image">
   `
   );
 }
}
...
function sendMessage() {
  const containerLayout = $('.messenger-layout .layout-main');
  temporaryMsgId += 1;
  ...
  if (inputValue.length > 0 || hasFile) {
    ...
    $.ajax({
      ...
      beforeSend: () => {
        ...
        // scroll to bottom
        scrollToBottom(containerLayout);
      ...
      success: (data) => {
        if (data.error > 0) {
          ...
          const tempMsgCardElement = containerLayout.find(
...
const messagesContainers = $('.messenger-layout .layout-main'),
  messagesElements = messagesContainers.find('.messages');
  ...
  if (messenger != 0 && !noMoreMessages ) {
      ...
      $.ajax({
          ...
          success: (data) => {
              ...
              if (messagesPage == 1) {
                  messagesElements.html(data.messages);
                  ...
                  messagesElements.prepend(data.messages);
...
activeStatusChannel.bind("pusher:member_added", function (member) {
 setActiveStatus(1);
//  alert(member.id);
  $('.messenger-layout[data-messenger-id="'+member.id+'"]').find('.status-active-render').addClass('active');
  $('.messenger-layout[data-messenger-id="'+member.id+'"]').find('.status-active-render .title').html('Đang hoạt động');


  $('.select-user-active[data-user-id="'+member.id+'"]').find('.status-active-render-index').addClass('active');
});

// Leaved
activeStatusChannel.bind("pusher:member_removed", function (member) {
 setActiveStatus(0);
 $('.messenger-layout[data-messenger-id='+member.id+']').find('.status-active-render').removeClass('active');
 $('.messenger-layout[data-messenger-id='+member.id+']').find('.status-active-render .title').html('Không hoạt động');

 $('.select-user-active[data-user-id="'+member.id+'"]').find('.status-active-render-index').removeClass('active');
});
...
function getContactsMain() {
  $.ajax({
      ...
      success: (data) => {
        // console.log(data);
          $(".chat-list-friend").append(data.contactsfriend);
          $('.chat-list-nofriend').append(data.contactsNoFriend)
          $('.chat-list-dating').append(data.contactsDating);
      },
      error: (error) => {
          console.error(error);
      }
...
function IDinfoMain(id) {
  ...
  if (messenger != 0) {
      ...
      success: (data) => {
          ...
          if(data.fetch.active_status === 1){
            $('.status-active-render').addClass('active');
            $('.status-active-render .title').html("Đang hoạt động")
          }

          // $('.status-active').html(html);
          $('.layout').attr("data-messenger-id",data.fetch.id);
          $('.messenger-messagingView').attr("data-messenger-id",data.fetch.id);
          $('#layout-avatar-user img').attr("src",data.fetch.avatar)
          $('#layout-name-user h6').html(data.fetch.first_name + " " + data.fetch.last_name)
...
function updateContactItem(user_id) {
 if (user_id != auth_id) {
   $.ajax({
     url: url + "/updateContactsItemMain",
     method: "POST",
     data: {
       _token: csrfToken,
       user_id,
     ...
     success: (data) => {
      ...
      $('.chat-list-friend').find('.route-layout[data-contact='+user_id+']').remove();
      $('.chat-list-nofriend').find('.route-layout[data-contact='+user_id+']').remove();
      $('.chat-list-dating').find('.route-layout[data-contact='+user_id+']').remove();


       $(".chat-list-friend").prepend(data.contactsfriend);
       $('.chat-list-nofriend').prepend(data.contactsNofriend)

      $(".chat-list-dating").prepend(data.contactsDating);
...
$(document).ready(function () {
  ...
  var storedId = localStorage.getItem('messengerId');
  layout(storedId);
 // get contacts list
 getContactsMain();
