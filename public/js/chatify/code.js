  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        // avatar photo
        var newUrl = "http://127.0.0.1:8082/";
        var id = data.fetch.id;
        var route = newUrl+ "profile/:id";
        route = route.replace(':id', id);
        
        // Set href attribute for the element with class 'show-listView'
        $('.show-listView').attr('href', route);
        $(".header-avatar img").attr("src", data.user_avatar);
        $('.user-name').attr('href', route);
  ...
  if (!contactsLoading && !noMoreContacts) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        if (contactsPage < 2) {
          $("#messenger-friend").html(data.contactsfriend);
          $('#messenger-nofriend').html(data.contactsNoFriend)
          $('#messenger-dating').html(data.contactsDating);
        } else {
       
          $("#messenger-friend").append(data.contactsfriend);
          $('#messenger-nofriend').append(data.contactsNoFriend)
          $('#messenger-dating').append(data.contactsDating);
        }
    ...
    $.ajax({
      ...
      success: (data) => {
        $(".listOfContacts")
          ...
          .remove();
        $("#messenger-friend").prepend(data.contactsfriend);
        $('#messenger-nofriend').prepend(data.contactsNofriend)

       $("#messenger-dating").prepend(data.contactsDating);
